...
export default function InvestorProfilePage({
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session: s } }) => {
      if (s?.user) setSession({ id: s.user.id, email: s.user.email! });
    });
  ...
  const inputStyle = {
    ...
    fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", padding: "8px 12px",
    ...
    <main style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <nav style={{ borderBottom: "1px solid var(--border)", padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "var(--bg)", zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          <Link href="/dashboard" style={{ fontSize: "10px", color: "var(--white-mid)", letterSpacing: "0.1em", textDecoration: "none" }}>// dashboard</Link>
          ...
          <div style={{ border: "1px solid var(--border2)", borderTop: "2px solid var(--amber)", background: "var(--bg2)", marginTop: "1px", padding: "24px", animation: "fadeUp 0.25s ease both" }}>
            ...
            {introStep === "auth" && (
              <div>
                ...
                <button onClick={handleGoogleSignIn} disabled={introLoading}
                  style={{ width: "100%", background: "var(--amber)", color: "#000", border: "none", fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", fontWeight: 700, letterSpacing: "0.06em", padding: "11px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
            ...
            {introStep === "form" && (
              <form onSubmit={handleSendIntro}>
                ...
                <button type="submit" disabled={introLoading || !form.founder_name || !form.company_name || !form.one_liner || !form.stage || !form.traction}
              ...
              <div style={{ animation: "fadeUp 0.25s ease both" }}>
                ...
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  ...
                  <button onClick={copyPassport}
                    style={{ background: copied ? "var(--amber)" : "var(--bg3)", color: copied ? "#000" : "var(--white-mid)", border: "1px solid var(--border2)", fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", fontWeight: 700, letterSpacing: "0.06em", padding: "10px 16px", cursor: "pointer", whiteSpace: "nowrap", transition: "all 0.15s" }}>
