...
export default function InvestorProfilePage({
  ...
  const [scopeSignal, setScopeSignal] = useState<{label: string; explanation: string} | null>(null);
  ...
  const handleSendIntro = async (e: React.FormEvent) => {
    ...
    // FIX 1: upsert with onConflict user_id â requires unique constraint (migration_v14.sql)
    ...
    fetch("/api/send-intro", {
      ...
      if (res && res.ok) {
        ...
        if (data.scopeLabel) {
          setScopeSignal({ label: data.scopeLabel, explanation: data.scopeExplanation });
        }
      }
    }).catch(console.error);
            ...
            <div style={{ border: "1px solid var(--border2)", marginBottom: "16px" }}>
              {!!inv.ticket_min && (
                <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <div style={{ padding: "9px 14px", fontSize: "12px" }}><span style={{ color: "var(--rasp)" }}>â¬{inv.ticket_min as number}K â â¬{inv.ticket_max as number}K</span></div>
                </div>
              )}
            ...
            {session && ((inv as { user_id?: string }).user_id === session.id || (ownerEmail && ownerEmail === session.email)) ? (
                  ...
                  <button
                    onClick={() => {
                      ...
                      navigator.clipboard.writeText(snippet).then(() => {
                        ...
                        if (btn) { btn.textContent = "â copied"; setTimeout(() => { btn.textContent = "embed â"; }, 2000); }
                      });
                    }}
                    id="embed-copy-btn"
              ...
              <div className="example-actions" style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
                <button
                  ...
                  {session ? "$ send intro (pre-filled) â" : "$ send intro â"}
                </button>
            ...
            {introStep === "auth" && (
              <div>
                ...
                <p style={{ fontSize: "12px", color: "var(--white-mid)", lineHeight: 1.7, marginBottom: "20px" }}>
                  Sign in once â your details save as a passport you can reuse with every investor.
                </p>
                <button onClick={handleGoogleSignIn} disabled={introLoading} style={{ width: "100%", background: "var(--amber)", color: "#000", border: "none", fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", fontWeight: 700, letterSpacing: "0.06em", padding: "11px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
                  ...
                  {introLoading ? "connecting..." : "$ continue with Google â"}
                </button>
        ...
        <div style={{ marginBottom: "12px" }}>
          ...
          <input required style={inputStyle} value={form.company_name} onChange={e => setForm(f => ({ ...f, company_name: e.target.value }))} placeholder="Carbonade" />
                ...
                <button
                  ...
                  {introLoading ? "sending..." : `$ send intro to ${inv.name as string} â`}
                </button>
              </form>
            )}
            {introStep === "done" && (
              <div style={{ animation: "fadeUp 0.25s ease both" }}>
                <div style={{ fontSize: "10px", letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--amber)", marginBottom: "12px" }}>// intro sent â</div>
                <p style={{ fontSize: "13px", color: "var(--white)", marginBottom: "6px" }}>
                  {inv.name as string} has been notified.
                </p>
                {scopeSignal && (
                  <div style={{ background: "var(--bg3)", border: "1px solid var(--border2)", borderRadius: "6px", padding: "12px 16px", marginBottom: "12px" }}>
                    <div style={{ fontSize: "11px", fontWeight: 600, color: scopeSignal.label.includes("Strong") ? "var(--green)" : scopeSignal.label.includes("Moderate") ? "var(--amber)" : "var(--white-dim)", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                      {scopeSignal.label}
                    </div>
                    <div style={{ fontSize: "12px", color: "var(--white-mid)", lineHeight: 1.6 }}>
                      {scopeSignal.explanation}
                    </div>
                  </div>
                )}
                <p style={{ fontSize: "12px", color: "var(--white-mid)", lineHeight: 1.7, marginBottom: "20px" }}>
